...
import { uploadImageController, getUserInfoController, listFollowerUserController, addFollowerUserController, deleteFollowerUserController, editFollowerUserController} from '../controller/User.controller';
...
// 这个方法应该还要修改 现在是image width height 期望可以有object-fit的功能 和自动适配分辨率功能(大图片降分辨率 小图不变)
router.post('/image/upload', upload.single('image'), authenticateJWT, uploadImageController)

router.post('/user/info', authenticateJWT, getUserInfoController);
...
router.post('/user/follower/edit', authenticateJWT, editFollowerUserController);
